...
export async function getDashboardState(personName = dashboardConfig.personName): Promise<DashboardState> {
  ...
  if (isGoogle && hasSourceError) {
    const snapshot = await readDashboardSnapshot(personName);
  ...
  if (isGoogle && !hasSourceError) await writeDashboardSnapshot(state, personName);
  return state;
}
